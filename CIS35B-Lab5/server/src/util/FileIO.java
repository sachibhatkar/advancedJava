...
public class FileIO {
    ...
    /**
     * building Automibile Object from input properties file.
     * @return
     */
    public Automobile buildAutomobileFromPropertiesFile(Properties properties) throws AutoException {
        String make = properties.getProperty("CarMake"); // Make Key from Properties File
        String model = properties.getProperty("CarModel"); // Model Key from Properties File
        float basePrice = Float.parseFloat(properties.getProperty("BasePrice")); // BasePrice Key  from Properties
        if (basePrice == 0) {
            throw new AutoException(AutoException.ErrorNumber.INVALID_BASE_PRICE);
        }
        Automobile automotive = new Automobile(make, model, basePrice);
        int numOptionSets = Integer
                .parseInt(properties.getProperty("OptionSetSize"));
        String alphabets = "0abcdefghijklmnopqrstuvwxyz"; // for matching optionName e.g. OptionName4a
        System.out.printf(" Number of OptionSets :  %d%n", numOptionSets);
        for (int i = 1; i <= numOptionSets; ++i) {
            ...
            //  Number of options for optionSets.
